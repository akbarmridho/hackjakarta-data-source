...
const finalConvert = async (data: FinalData[]) => {
  ...
  await fs.writeFile("final_banget.csv", csv, { encoding: "utf-8" });
};
...
(async function main() {
  const raw = await fs.readFile("./final.json", {
    encoding: "utf-8",
  });
  ...
  const dirs = await fs.readdir("temp");

  for (const dir of dirs) {
    const raw = await fs.readFile(`temp/${dir}`, { encoding: "utf-8" });
