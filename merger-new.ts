...
const finalConvert = async (data: FinalData[]) => {
  ...
  await fs.writeFile("additional.csv", csv, { encoding: "utf-8" });
};
...
(async function main() {
  ...
  const dirs = await fs.readdir("batch_classified_new");

  const result: FinalData[] = [];

  for (const dir of dirs) {
    console.log(`reading dir ${dir}`);
    const batchRaw = await fs.readFile(`batch_classified_new/${dir}`, {
      encoding: "utf-8",
    });

    const parsed = parse(batchRaw.trim(), {
      delimiter: ";",
      relaxQuotes: true,
      columns: [
        "menuId",
    ...
    await fs.writeFile("additional.json", JSON.stringify(result), {
      encoding: "utf-8",
    });
